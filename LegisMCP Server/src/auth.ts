...
export async function callback(c: Context<{ Bindings: Env & { OAUTH_PROVIDER: OAuthHelpers } }>) {
	...
	// Register the user in LegisAPI database
	try {
		const registerResponse = await fetch(`${c.env.API_BASE_URL}/api/users/register`, {
			method: 'POST',
			headers: {
		...
		// Continue even if registration fails - it will auto-register on first API call
	}
                ...
                <div class="container">
                    <div class="card">
                        <div class="header">
                            ...
                            <h1>LegislativeMCP - Authorization Request</h1>
                            <a href="${clientUri}" target="_blank" rel="noopener noreferrer" class="app-link">${clientName}</a>
                        </div>

                        <p class="description">
                            <strong>${clientName}</strong> is requesting permission to access the <strong>LegislativeMCP API</strong> using your
